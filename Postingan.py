...
class my_posts(tk.Tk):
    ...
    def open_Thrifting_form(self, callback):
        """Open the thrifting form window."""
        self.withdraw()  # Hide the current window
        from BiodataBajuThrifting import BiodataBajuThrifting
        thrifting_form = BiodataBajuThrifting(callback)
        thrifting_form.mainloop()
        self.deiconify()  # Show the current window after the form is done

    def add_post(self):
        def continue_add_post():
            ...
            self.destroy()
            import Postingan
            Postingan.my_posts(self.username, self.account_type)

        self.open_Thrifting_form(continue_add_post)
