...
class AI_Thrifting(tk.Tk):
    ...
    def on_exit(self):
        """Handle cleanup and navigate back to Postingan."""
        if self.cap is not None and self.cap.isOpened():
            self.cap.release()
        self.destroy()  # Close the current window
        from Postingan import my_posts
        app = my_posts(username=self.username, account_type=self.account_type)
        app.mainloop()


if __name__ == "__main__":
    app = AI_Thrifting(username="User123", account_type="Seller")
    app.mainloop()
